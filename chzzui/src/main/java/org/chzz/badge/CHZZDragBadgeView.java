 ...
 * 描述:
 */
class CHZZDragBadgeView extends View {
    private static final String TAG = CHZZDragBadgeView.class.getSimpleName();
    private CHZZBadgeViewHelper mBadgeViewHelper;
    private Paint mBadgePaint;
    private WindowManager mWindowManager;
    private WindowManager.LayoutParams mLayoutParams;
    private int mStartX;
    private int mStartY;
    private CHZZExplosionAnimator mExplosionAnimator;
    ...
    public CHZZDragBadgeView(Context context, CHZZBadgeViewHelper badgeViewHelper) {
        super(context);
    ...
    private void initStick() {
        mMaxDragRadius = CHZZBadgeViewUtil.dp2px(getContext(), 10);
        mDragStickRadiusDifference = CHZZBadgeViewUtil.dp2px(getContext(), 1);
    }
    ...
    private void drawStick(Canvas canvas) {
        ...
        // 通过几何图形工具获取交点坐标
        mDragPoints = CHZZBadgeViewUtil.getIntersectionPoints(mDragCenter, mDragRadius, lineK);
        mStickPoints = CHZZBadgeViewUtil.getIntersectionPoints(mStickCenter, currentStickRadius, lineK);

        // 3. 获取控制点坐标
        mControlPoint = CHZZBadgeViewUtil.getMiddlePoint(mDragCenter, mStickCenter);

        // 保存画布状态
        canvas.save();
        canvas.translate(0, -CHZZBadgeViewUtil.getStatusBarHeight(mBadgeViewHelper.getRootView()));
    ...
    private float getCurrentStickRadius() {
         ...
         */
        float distance = CHZZBadgeViewUtil.getDistanceBetween2Points(mDragCenter, mStickCenter);
        distance = Math.min(distance, mDismissThreshold);
        float percent = distance / mDismissThreshold;
        return CHZZBadgeViewUtil.evaluate(percent, mStickRadius, mStickRadius * 0.2f);
    }
    ...
    private void handleActionMove(MotionEvent event) {
        if (mExplosionAnimator == null && getParent() != null) {
            ...
            // 处理断开事件
            if (CHZZBadgeViewUtil.getDistanceBetween2Points(mDragCenter, mStickCenter) > mDismissThreshold) {
                mDismissAble = true;
                postInvalidate();
    ...
    private void startSpringAnim() {
        ...
        springAnim.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator mAnim) {
                ...
                PointF p = CHZZBadgeViewUtil.getPointByPercent(startReleaseDragCenter, mStickCenter, percent);
        ...
        springAnim.setDuration(CHZZExplosionAnimator.ANIM_DURATION / 2);
        springAnim.start();
    }
    ...
    private void startDismissAnim(int newX, int newY) {
        ...
        Bitmap badgeBitmap = CHZZBadgeViewUtil.createBitmapSafely(this, rect, 1);
        if (badgeBitmap == null) {
            removeSelf();
            mBadgeViewHelper.endDragWithDismiss();
            return;
        ...
        mExplosionAnimator = new CHZZExplosionAnimator(this, rect, badgeBitmap);
    ...
    private int getNewStartY(float rawY) {
        ...
        int newStartY = (int) rawY - badgeHeight / 2 - CHZZBadgeViewUtil.getStatusBarHeight(mBadgeViewHelper.getRootView());
