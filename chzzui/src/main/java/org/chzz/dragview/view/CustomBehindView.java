...
public class CustomBehindView extends GridView {
	...
	public CustomBehindView(Context context, CHZZDragView CHZZDragView) {
		super(context);
		this.mContext = context;
		this.mCHZZDragView = CHZZDragView;
		this.setNumColumns(CHZZDragView.COLUMNUM);
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		switch (ev.getAction()) {
			case MotionEvent.ACTION_DOWN:
				...
				if (mCHZZDragView.isEditModel() && tempPosit != mDragPosition) {
					mCHZZDragView.setEditModel(false, 0);
					return true;
				}
	...
	public void deletInfo(int position, DragIconInfo iconInfo) {
		...
		mCHZZDragView.deletHomePageInfo(iconInfo);
	}
